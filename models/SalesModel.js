...
export default class SalesModel {
  ...
  static async saveToReports(salesData, reportType = 'daily', reportName = 'Sales Report') {
    ...
    try {
      ...
      const total_items = salesData.reduce(
        (sum, sale) => sum + (parseInt(sale.items_count) || 0),
        0
      );
  ...
  static async exportSales(format = 'csv', filters = {}) {
    ...
    if (format === 'csv') {
      ...
      return this.convertToExcel(sales);
    }
  ...
  static convertToCSV(data) {
    if (data.length === 0) {
      return 'Sale Code,Date,Time,Customer,Items Count,Total Amount,Payment Method\n';
    }
